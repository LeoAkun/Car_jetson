...
public:
            ...
            ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
    {
        ...
        pubLaserCloudInfo = create_publisher<lio_sam::msg::CloudInfo>(
            "lio_sam/deskew/cloud_info", qos); // 发布去畸变，待提取特征的点云
...
void printPointCloud2Info(const sensor_msgs::msg::PointCloud2::SharedPtr& msg)
{
    std::cout << "=== PointCloud2 Info ===" 
              << std::endl;
    ...
    std::cout << "Point step: " << msg->point_step 
              << ", Row step: " << msg->row_step 
              << std::endl;

    std::cout << "Is dense: " << (msg->is_dense ? "true" : "false") 
              << std::endl;
   ...
   size_t num_points = std::min<size_t>(5, msg->width * msg->height);

    std::cout << "=== First " << num_points << " points ===" << std::endl;

    for (size_t i = 0; i < num_points; ++i)
    {
        std::cout << "Point[" << i << "]: ";
        const uint8_t* point_ptr = &msg->data[i * msg->point_step]; // 指向第 i 个点的起始字节

        for (const auto& field : msg->fields)
        {
            const uint8_t* field_ptr = point_ptr + field.offset; // 字段实际存储地址

            // 这里只处理常见的 FLOAT32、UINT8、INT32 等类型，其他类型可扩展
            switch (field.datatype)
            {
                ...
                {
                    uint8_t value = *field_ptr;
                    std::cout << field.name << "=" << static_cast<int>(value) << " ";
                    break;
                }
                case sensor_msgs::msg::PointField::INT32:
                {
                    int32_t value;
                    std::memcpy(&value, field_ptr, sizeof(int32_t));
                    std::cout << field.name << "=" << value << " ";
                    break;
                }
                ...
                {
                    uint32_t value;
                    std::memcpy(&value, field_ptr, sizeof(uint32_t));
                    std::cout << field.name << "=" << value << " ";
                    break;
                }
                default:
                    std::cout << field.name << "=<unhandled> ";
            }
        }
        std::cout << "\n" << std::endl ;
    }
...
void printPCLCloudInfo(const pcl::PointCloud<PointXYZIRT>::Ptr& cloud)
{
    std::cout << "=== PCL PointCloud Info ===" << std::endl;
    std::cout << "Width: " << cloud->width 
              << ", Height: " << cloud->height 
              << ", Points: " << cloud->points.size() << std::endl;
    std::cout << "Is dense: " << (cloud->is_dense ? "true" : "false") << std::endl;

    size_t num_points = std::min<size_t>(5, cloud->points.size());
    std::cout << "=== First " << num_points << " points ===" << std::endl;
    ...
    for (size_t i = 0; i < num_points; ++i)
    {
        ...
        // 模拟 PointCloud2 的字段遍历格式打印
        struct Field {
            std::string name;
            const void* ptr;
            enum Type { FLOAT32, UINT8 } type;
        };
        Field fields[] = {
            {"x", &p.x, Field::FLOAT32},
            {"y", &p.y, Field::FLOAT32},
            {"z", &p.z, Field::FLOAT32},
            {"intensity", &p.intensity, Field::FLOAT32},
            {"ring", &p.ring, Field::UINT8},
            {"time", &p.time, Field::FLOAT32}
        };
        ...
        for (const auto& f : fields)
        {
            switch (f.type)
            {
                ...
                {
                    float value;
                    std::memcpy(&value, f.ptr, sizeof(float));
                    std::cout << f.name << "=" << std::fixed << std::setprecision(6) << value << " ";
                    break;
                }
                case Field::UINT8:
                {
                    uint8_t value;
                    std::memcpy(&value, f.ptr, sizeof(uint8_t));
                    std::cout << f.name << "=" << static_cast<int>(value) << " ";
                    break;
                }
            }
        }
    ...
    {
        // 入队 cache point cloud
        cloudQueue.push_back(*laserCloudMsg);
        if (cloudQueue.size() <= 2)
            return false;

        // 取出最早的一帧 convert cloud
        ...
        {   
            ...
            // 打印转换之前的laser数据
            // printPointCloud2Info(laserCloudMsg);

            // 转换为pcl点云格式XYZIRT字段
            pcl::moveFromROSMsg(currentCloudMsg, *laserCloudIn);  

            // 打印转换之后的laser数据
            // printPCLCloudInfo(laserCloudIn);
        }
