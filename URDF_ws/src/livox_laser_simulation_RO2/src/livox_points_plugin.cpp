...
namespace gazebo
{
    ...
    void LivoxPointsPlugin::OnNewLaserScans() {
        ...
        // 然后手动修改字段（通过直接操作 PointCloud2 的字段数组）
        cloud2.fields.push_back(
            sensor_msgs::msg::PointField()
            .set__name("ring")
            .set__offset(cloud2.point_step)  // 新字段的偏移量
            .set__datatype(sensor_msgs::msg::PointField::UINT16)
            .set__count(1)
        );

        // 更新 point_step（每个点的总字节数）
        cloud2.point_step += 2;  // UINT16 占 2 字节
