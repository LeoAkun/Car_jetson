...
void Pcd2PgmNode::passThroughFilter(double thre_low, double thre_high, bool flag_in)
{
  ...
  RCLCPP_INFO(
    get_logger(), "Before PassThrough: z range [%.3f, %.3f], total points: %lu",
    z_min, z_max, pcd_cloud_->points.size());
  RCLCPP_INFO(
    get_logger(), "PassThrough filter settings: thre_low=%.3f, thre_high=%.3f, setNegative=%s",
    thre_low, thre_high, flag_in ? "true" : "false");
  ...
  for (const auto & point : cloud_after_pass_through_->points) {
    z_min = std::min(z_min, static_cast<double>(point.z));
    z_max = std::max(z_max, static_cast<double>(point.z));
  }
  RCLCPP_INFO(
    get_logger(), "After PassThrough: z range [%.3f, %.3f], remaining points: %lu",
    z_min, z_max, cloud_after_pass_through_->points.size());
}
